...
from system.flcore.optimizers.fedoptimizer import FedLMOptimizer
from system.utils.privacy import *


class clientFedLM(Client):
    def __init__(self, args, id, train_samples, test_samples, **kwargs):
        super().__init__(args, id, train_samples, test_samples, **kwargs)

        self.optimizer = FedLMOptimizer(self.model.parameters(), lr=self.learning_rate)
    ...
    def train(self):
        ...
        for epoch in range(max_local_epochs):
            for i, (x, y) in enumerate(trainloader):
                ...
                loss.backward()
                self.optimizer.step(self.privacy, self.dp_sigma, self.laplacian, self.ls_sigma, self.global_c, self.client_c, self.device)
